    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: "1240px" }}>
        <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            {navItems.map((item, index) => (
              ...
              >
                <motion.a
                  ...
                  className="flex items-center space-x-1 px-3 py-2 rounded-lg text-[#141414] hover:text-[#7d7d7d] transition-colors duration-200"
                >
                  <span>{item.name}</span>
                  {item.dropdown && (
                ...
                <AnimatePresence>
                  {item.dropdown && activeDropdown === item.name && (
                    <motion.div
                      ...
                      className="absolute top-full left-0 mt-2 w-64 bg-[#ffffff] border border-[#e7d8c5] rounded-xl shadow-lg z-50"
                    >
                      {item.dropdown.map((dropdownItem, dropdownIndex) => (
                        <motion.a
                          key={dropdownIndex}
                          href={dropdownItem.href}
                          whileHover={{ x: 4, backgroundColor: "#e7d8c5" }}
                          transition={{ duration: 0.15 }}
                          className="block px-4 py-3 text-[#141414] hover:text-[#7d7d7d] transition-colors first:rounded-t-xl last:rounded-b-xl"
                        >
                          {dropdownItem.name}
                        </motion.a>
                      ))}
          ...
          <motion.button
            whileTap={{ scale: 0.95 }}
            className={`lg:hidden p-2 text-[#141414] hover:bg-[#e7d8c5] rounded-lg transition-colors ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        ...
        {isOpen && (
          <>
            <motion.div
              ...
              className="fixed inset-0 bg-[#141414]/50 z-[55] lg:hidden"
            ...
            <motion.div
              ...
              className="fixed top-0 left-0 h-screen w-80 max-w-[85vw] bg-[#ffffff] shadow-xl z-[60] lg:hidden overflow-y-auto"
            >
              <div className="p-6">
                ...
                <nav className="space-y-2">
                  {navItems.map((item, index) => (
                    <div key={index}>
                      ...
                      >
                        <div
                          className="flex items-center justify-between p-3 rounded-lg text-[#141414] hover:bg-[#e7d8c5] transition-colors duration-200 cursor-pointer"
                          onClick={() => {
                            if (item.dropdown) {
                        ...
                        >
                          ...
                          {item.dropdown && (
                            <span className={`material-icons text-[#7d7d7d] transition-transform duration-200 ${
                            ...
                            >
                              {item.dropdown.map((dropdownItem, dropdownIndex) => (
                                <a
                                  key={dropdownIndex}
                                  href={dropdownItem.href}
                                  className="block p-2 pl-4 text-[#7d7d7d] hover:text-[#141414] hover:bg-[#e7d8c5]/50 rounded transition-colors duration-200"
                                  onClick={() => setIsOpen(false)}
                                >
                                  {dropdownItem.name}
                                </a>
